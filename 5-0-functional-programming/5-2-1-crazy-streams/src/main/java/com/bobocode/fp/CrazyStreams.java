...
public class CrazyStreams {
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream()
                .collect(Collectors.groupingBy(account -> account.getSex() == Sex.MALE));
    }
    ...
    public Map<String, BigDecimal> collectBalancesByEmailForAccountsCreatedOn(int year) {
       return accounts.
               stream().
               filter(account -> account.getCreationDate().
                       getYear() == year).
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream().flatMap(
                account -> account.getFirstName().chars().mapToObj(c->(char) c)).
                collect(Collectors.groupingBy(
                character -> character,
                        Collectors.counting()
        ));
        ...
        return accounts.stream()
                .flatMap(account -> {
                    ...
                    return Stream.of(firstName, lastName)
                            ...
                            .flatMap(name -> name.toLowerCase().chars().mapToObj(c -> (char) c)); // Convert names to lowercase character streams
                })
                .collect(Collectors.groupingBy(
                        character -> character,
                        Collectors.counting()
